...
def scope_note(terms: Dict[str, int], vector_used: bool, vector_count: int, vector_note: str = "") -> str:
    ...
    if vector_used:
        ...
        # Технические подробности (404, отсутствующая коллекция) остаются в журнале запуска,
        # а в отчёт идёт одна нейтральная фраза без деталей.
        note += " Дополнительный смысловой поиск по этому датасету не применялся."
    return note
