...
PRESETS: List[Dict[str, Any]] = [
    {
        ...
        "instruction": (
            ...
            "с цитатами и ссылками, детали (продукты, точки, суммы). Перед выводами обязательно прочитай тексты сообщений периода "
            "инструментом analyze_texts: темы, доли и цитаты должны попасть в отчёт отдельным разделом. "
    ...
    },
    {
        ...
        "instruction": (
            ...
            "Кроме этого прочитай тексты сообщений периода инструментом analyze_texts (темы с долями и цитатами) — "
            "он обязателен перед сборкой отчёта. "
    ...
    },
    {
        ...
        "description": (
            "Готовая цепочка из 15 шагов: поисковые слова подтемы → ИИ-аналитика → тональность, авторы и рейтинг СМИ, "
            "цепочки авторов, примеры негатива и позитива → ЧТЕНИЕ ТЕКСТОВ локальной моделью (темы, доли, цитаты) → "
            "три графика → выводы ИИ → отчёт DOCX/PDF с темами и цитатами. "
            "Свою подтему (поисковые слова) укажите в шагах 1, 4, 5, 6, 7 и 8 вместо слова «просрочка»."
        ),
        "instruction": (
            "Собери отчёт по подтеме: тональность, авторы, цепочки авторов, негатив, позитив, темы и цитаты из текстов, выводы."
        ),
        "tools": [
            "search_messages",
            "analyze_texts",
        ...
        "steps": [
            ...
            {"kind": "tool", "title": "8. Чтение текстов периода (обязательно)", "tool": "analyze_texts",
             "args": {"tone": "all", "limit": 90, "focus": "подтема: просрочка — причины недовольства, сроки, возврат денег"},
             "save_as": "texts"},
            ...
            {"kind": "llm", "title": "9. Раздел «Тональность»", "save_as": "sum_tone", "max_tokens": 900,
             "prompt": (
                 ...
                 "ЧТЕНИЕ ТЕКСТОВ (локальная модель): прочитано {{texts.messages_analyzed}} сообщений, "
                 "тональность среза {{texts.tone_counts}}.\n"
                 "Аналитический текст по текстам: {{texts.summary}}\n"
                 "Темы с долями, тональностью и цитатами: {{texts.topics}}\n"
                 "Ключевые сообщения: {{texts.highlights}}\n\n"
                 ...
                 "5) что хвалят в позитиве (3 пункта). Динамику и графики поясняй ЧЕРЕЗ ТЕМЫ И ЦИТАТЫ из чтения текстов, "
                 "а не только через статистику. Только факты из данных, без вводных фраз, объём 5–8 абзацев."
             )},
            {"kind": "llm", "title": "10. Раздел «Авторы и цепочки»", "save_as": "sum_authors", "max_tokens": 900,
             "prompt": (
                 "Подтема: {{search.phrase}}. Датасет: {{search.index_name}}, период {{search.period.from}} — {{search.period.to}}.\n\n"
            ...
            {"kind": "llm", "title": "11. Выводы и рекомендации", "save_as": "sum_conclusions", "max_tokens": 800,
             "prompt": (
                 "Подтема: {{search.phrase}}. Найдено {{search.messages_found}} сообщений, тональность: {{search.tonality}}.\n\n"
                 "Раздел «Тональность»:\n{{sum_tone.text}}\n\n"
                 "Раздел «Авторы и цепочки»:\n{{sum_authors.text}}\n\n"
                 "Темы из чтения текстов с долями: {{texts.topics}}\n"
                 "Ключевые сообщения из текстов: {{texts.highlights}}\n\n"
                 ...
                 "по каждой причине недовольства опирайся на темы и цитаты из чтения текстов."
             )},
            {"kind": "report", "title": "12. Сборка отчёта DOCX/PDF", "save_as": "report",
             "report_title": "Отчёт по подтеме: {{search.phrase}}",
             "subtitle": "Датасет {{search.index_name}}, период {{search.period.from}} — {{search.period.to}}",
             "folder": "Отчёт по подтеме",
             "sections": [
                 {"heading": "Темы и цитаты из текстов сообщений", "text": "{{texts.summary}}",
                  "findings": "{{texts.report_section.findings}}",
                  "highlights": "{{texts.report_section.highlights}}",
                  "citations": "{{texts.report_section.citations}}"},
