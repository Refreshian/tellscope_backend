...
def _plan_prompt(text: str, index: Optional[int], dataset_name: str) -> str:
    return (
        ...
        "Правило для сводных задач: если задача просит «свести», «сравнить по месяцам или годам», «годовой отчёт» "
        "или «на основе ранее построенных отчётов», первым шагом ставь read_reports — чтение готовых итогов "
        "прошлых запусков (компактный JSON рядом с каждым отчётом), и только если итогов нет — чтение сырых "
        "данных. В таком плане отметь в cautions, по чему собрана сводка.\n\n"
