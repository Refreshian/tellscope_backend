...
async def run_llm_query(task_data: dict):
    ...
    try:
        ...
        all_labels = [label if label else "Unlabelled" for label in llm_labels]
        all_labels = [llm_labels[topic+topic_model._outliers] if topic != -1 else "Unlabelled" for topic in topics]

        # # Run the visualization
        # datamapplot = datamapplot.create_plot(
        #     embeddings_umap,
        #     all_labels,
        #     label_font_size=11,
        ...
        # datamapplot.savefig("BBBBBBBdatamapplot-basic_usage-example.png", bbox_inches="tight")
